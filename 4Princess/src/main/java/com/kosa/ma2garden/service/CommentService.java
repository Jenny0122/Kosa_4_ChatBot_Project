...
public class CommentService {
	...
	public List<CommentDTO> getAllComment() {
		...
		for (Comment comment : commentRepository.findAll()) {
			CommentDTO commentDTO = CommentDTO.builder().co_no(comment.getCo_no()).comment(comment.getComment()).day(comment.getDay())
					.board_no(comment.getBoard_no()).user_no(comment.getUser_no()).build();
			list.add(commentDTO);
		}

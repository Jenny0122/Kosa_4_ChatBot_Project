...
public class DryPlantService {
	...
	public List<DryPlantDTO> getAllDryplants(){  //this
		
		
		List<DryPlantDTO> list = new ArrayList<DryPlantDTO>();
		
		for (DryPlant dryplant : dryplantrepository.findAll()) {
			DryPlantDTO dryplantDTO = new DryPlantDTO(
					dryplant.getNo(),
					dryplant.getPlanttName(),
					dryplant.getChartrInfo(),
					dryplant.getFlwrInfo(),
					dryplant.getLightInfo(),
					dryplant.getGrwtInfo(),
					dryplant.getWaterCycleInfo(),
					dryplant.getBatchPlaceInfo()
					);

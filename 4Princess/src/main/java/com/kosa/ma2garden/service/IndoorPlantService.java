...
public class IndoorPlantService {
	...
	public List<IndoorPlantDTO> getAllIndoorplants() {
		...
		for (IndoorPlant indoorplant : indoorplantrepository.findAll()) {
			IndoorPlantDTO indoorplantDTO = new IndoorPlantDTO(indoorplant.getNo(), indoorplant.getName(),
					indoorplant.getLef_stle_info(), indoorplant.getPrpgt_era_info(), indoorplant.getFrtlzr_info(),
					indoorplant.getSoil_info(), indoorplant.getFnclty_info());
